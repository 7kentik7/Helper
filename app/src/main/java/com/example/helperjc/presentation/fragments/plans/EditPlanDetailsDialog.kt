...
import com.example.helperapp.databinding.EditPlanDialogBinding
...
import com.example.helperapp.presentation.fragments.EditPlanDetailsDialogArgs
...
class EditPlanDetailsDialog : DialogFragment(), DatePickerFragment.OnDateSelectedListener {
    @Inject
    lateinit var viewModelFactory: ViewModelFactory
    private val viewmodel by lazy {
        ViewModelProvider.create(this, viewModelFactory)[EditPlanDetailsViewModel::class]
    }
    private val args by navArgs<EditPlanDetailsDialogArgs>()
    private var _binding: EditPlanDialogBinding? = null
    private val binding: EditPlanDialogBinding
        get() = _binding
            ?: throw RuntimeException("_root_ide_package_.com.example.helperapp.databinding.EditPlanDialogBinding is null")
    ...
    override fun onCreateView(
        ...
        _binding = EditPlanDialogBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewmodel.state.collect {
                    when (it) {
                        is EditPlanDetailsState.DataLoaded -> {
                            ...
                            binding.radioBottonBlueColor.isChecked =
                                it.color == PlanColor.BLUE
                            binding.radioBottonTealColor.isChecked =
                                it.color == PlanColor.TEAL
                            binding.radioBottonPurpleColor.isChecked =
                                it.color == PlanColor.PURPLE
                            binding.radioBottonPinkColor.isChecked =
                                it.color == PlanColor.PINK
                            binding.radioBottonOrangeColor.isChecked =
                                it.color == PlanColor.ORANGE
                            binding.radioBottonImeColor.isChecked =
                                it.color == PlanColor.IME
                        }

                        is EditPlanDetailsState.Error -> {
                            makeToast(it.messageResId)
                        }

                        EditPlanDetailsState.Initial -> {
                            viewmodel.loadData(args.planId)
                        }

                        EditPlanDetailsState.Success -> {
